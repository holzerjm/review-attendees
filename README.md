...
This Google Apps Script adds a custom menu to your Google Sheet, allowing you to quickly "Accept" or "Reject" one or more selected rows. It is designed to update status columns, add a reviewer email, and timestamp the decision.
...
1.  In your Google Sheet, go to the menu **Extensions > Apps Script**. A new browser tab will open with the script editor.

### Step 3: Add the Script File

1.  In the script editor, you will see a default file named `Code.gs`. Click the three dots next to it and rename it to **`reviewNominees.gs`**.
2.  Delete any placeholder code inside it.
3.  Copy all the code from the `reviewNominees` file in this repository and paste it into this file.
...
8.  A new screen will show what the script needs. It will ask for permission to **"See, edit, create, and delete all your Google Sheets spreadsheets"**.
9.  Click **"Allow"**.
